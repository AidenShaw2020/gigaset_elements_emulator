...
1. Build the loader once: `./tools/build_uart_loader.sh` (needs `git`, `docker`
   and `patch`). It produces `452dump.bin` in the current directory - see
...
[`tools/patches/452dump-flash-read-only.patch`](tools/patches/452dump-flash-read-only.patch),
...
one exception: it needs the loader built by `tools/build_uart_loader.sh`, which
...
UART loader built by `tools/build_uart_loader.sh` are Gigaset/Dialog Semiconductor
